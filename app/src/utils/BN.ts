...
class BNSubType extends BN {
  ...
  constructor(number: number | string | number[] | Uint8Array | Buffer | BN | bigint) {
    super(
...
export class CurrencyBalance extends BN {
  decimals: number
  constructor(number: number | string | number[] | Uint8Array | Buffer | BN | bigint, decimals: number) {
    super(
      (typeof number === 'object' && 'toPrimitive' in number) || typeof number === 'bigint' || BN.isBN(number)
        ? number.toString()
        : number
