...
export function PoolDetailHeader({ actions }: Props) {
  ...
  const pool = undefined // usePool(pid)
  const { data: metadata, isLoading } = {data: undefined, isLoading: false}//usePoolMetadata(pool)
  const isTinlakePool = false // pool.id.startsWith('0x')
